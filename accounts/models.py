...
class AgentReferral(models.Model):
    ...
    amount        = models.PositiveIntegerField(default=750)       # G gagnés
...
class GiftPaymentLink(models.Model):
    ...
    student_contact = models.CharField(max_length=255, blank=True, default='')  # email ou tél stocké à la génération
...
class DailyUsage(models.Model):
    """Compteurs journaliers pour limiter l'usage (gratuit + plafond IA global)."""
    ...
    ai_request_count  = models.PositiveIntegerField(default=0)
...
class GroupMessage(models.Model):
    ...
    video       = CloudinaryField('group_video', folder='bacia/group_chat/videos', blank=True, null=True)
    created_at  = models.DateTimeField(auto_now_add=True)
    is_system   = models.BooleanField(default=False)
    quiz_data   = models.JSONField(default=None, null=True, blank=True)
    reply_to    = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True, related_name='replies')
    mentions    = models.ManyToManyField(User, blank=True, related_name='mentioned_group_messages')
...
class GroupMessageNotification(models.Model):
    """Notification ciblée pour le groupe: uniquement mentions et réponses."""
    REASON_CHOICES = [
        ('reply', 'Réponse'),
        ('mention', 'Mention'),
        ('everyone', 'Mention tout le monde'),
    ]
    message = models.ForeignKey(GroupMessage, on_delete=models.CASCADE, related_name='notifications')
    recipient = models.ForeignKey(User, on_delete=models.CASCADE, related_name='group_message_notifications')
    ...
    def __str__(self):
        return f"{self.recipient.username} <- group msg {self.message_id} ({self.reason})"


class MessageDeletion(models.Model):
    """Tracks deletions of messages. Can represent "delete for all" or "delete for me".
    Uses GenericForeignKey so it can reference FriendMessage or GroupMessage.
    """
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')
    deleted_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='deleted_messages')
    ...
    def __str__(self):
        who = self.deleted_by.username if self.deleted_by else 'unknown'
        return f"Deleted {self.content_type_id}/{self.object_id} by {who} (for_all={self.for_all})"


class GroupChatQuizAttempt(models.Model):
    """Tentative de réponse à un quiz posté dans le chat de groupe."""
    message          = models.ForeignKey(GroupMessage, on_delete=models.CASCADE, related_name='quiz_attempts')
    user             = models.ForeignKey(User, on_delete=models.CASCADE, related_name='group_quiz_attempts')
    ...
    def __str__(self):
        return f"{self.user.username} → quiz msg {self.message_id} ({'✓' if self.is_correct else '✗'})"
