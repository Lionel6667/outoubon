...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if not all([first_name, last_name, contact, pwd1]):
            error = 'Remplis tous les champs obligatoires (prénom, nom, contact et mot de passe).'
        elif pwd1 != pwd2:
...
def signup_step2_view(request):
    ...
    if request.method == 'POST':
        ...
        if not school_name:
            error = 'Entre le nom de ton école.'
        elif not serie:
...
def complete_profile_view(request):
    ...
    if request.method == 'POST':
        ...
        profile.serie  = serie
        profile.langue_etrangere = langue_etrangere
...
def api_device_check(request):
    ...
    if not profile.device_fingerprint:
        ...
        return JsonResponse({
            'ok': True,
            'device_changed': False,
            'active_device': True,  # this IS the currently registered device
        ...
        if profile.pending_device_fingerprint == fp:
            ...
            return JsonResponse({
                'ok': True,
                'device_changed': False,
                'active_device': False,  # new device, not yet the active one
                'pending_switch': True,
                'grace_remaining': 300,
            })
...
def api_device_status(request):
    ...
    if profile.pending_device_fingerprint and profile.pending_device_at:
        ...
        if elapsed >= 300:
            # Grace period expired — commit the switch NOW
            current_session_key = request.session.session_key

            if profile.pending_device_fingerprint == fp:
                # New device calling: commit the switch for real
                ua = request.META.get('HTTP_USER_AGENT', '')[:500]
                ...
                # Old device calling: kick it — invalidate its session now
                result['kicked'] = True
                result['pending_switch'] = False
                try:
                    request.session.flush()
