"""
Service IA BacIA — DeepSeek API (compatible OpenAI)
Modèle principal : deepseek-v4-pro   (cours, exercices, examens blancs — 10% cas complexes)
Modèle léger     : deepseek-v4-flash (chat, QCM, progression, conseils — 90% des cas)
"""
...
def _call_json(prompt: str, system: str = '', max_tokens: int = 3000) -> str:
    """Appel DeepSeek Pro pour génération JSON complexe — exercices, examens, quiz avancés."""
    messages: list = []
    if system:
        messages.append({"role": "system", "content": system})
    messages.append({"role": "user", "content": prompt})
...
VISION_MODEL = 'deepseek-v4-flash'  # DeepSeek V4-Flash supporte le multimodal
...
def _generate_mcq_options(intro: str, first_question: str, subject: str) -> tuple[list[str], str]:
    ...
    # Première tentative avec DeepSeek
    try:
...
CORRECT: X"""
        ...
        if resp and resp.choices and len(resp.choices) > 0:
            ...
            if raw and raw.strip():
                print(f"[DEBUG] DeepSeek response OK: {raw[:100]}")
    ...
    if subject == 'maths':
        ...
        try:
            ...
            def _render_maths_exercise(ex: dict) -> str:
                ...
                # Suites : compléter jusqu'à 4 sous-questions minimum
                elif _is_suite(ex):
                    needed = max(0, 4 - _question_count(ex))
                    extras = [
                        'b) Étudier la monotonie de la suite $(u_n)$.',
                        'c) La suite $(u_n)$ est-elle convergente ? Si oui, calculer sa limite.',
                        'd) Montrer par récurrence que $u_n > 0$ pour tout $n \\in \\mathbb{N}$.',
                        'e) Calculer les quatre premiers termes et conjecturer le comportement de la suite.',
                    ]
                    if needed > 0:
                        parts.extend(extras[:needed])

                # Statistiques : compléter jusqu'à 4 sous-questions minimum
                elif _is_stats(ex):
                    needed = max(0, 4 - _question_count(ex))
                    extras = [
                        'b) Calculer la moyenne $\\bar{x}$ et la médiane $M_e$ de cette série.',
                        'c) Calculer la variance $\\sigma^2$ et l\'écart-type $\\sigma$.',
                        'd) Déterminer le premier quartile $Q_1$ et le troisième quartile $Q_3$.',
                        'e) Interpréter les résultats obtenus dans le contexte du problème.',
                    ]
                    if needed > 0:
                        parts.extend(extras[:needed])
                    ...
                    'sections': [
                        {
                            'label': (
                                ...
                                f'{it.get("theme", it.get("chapter", "Mathématiques"))} — {it.get("source_display", it.get("source", ""))}'
                            ),
                            'type': 'open', 'pts': 20,
                            'items': [{'text': _fix_latex(_global_format_tables(it.get('_rendered_text') or _fmt_exercice(it) or it.get('enonce', it.get('intro', '')))), 'answer': 'Réponse développée attendue.', 'pts': 20, 'no_num': True}],
                        }
...
def generate_exam_exercise(subject: str, chapter: str, exam_context: str = '', chapter_rule: dict | None = None) -> dict:
    ...
    text = _call(prompt, max_tokens=1200)  # Pro model — page exercice
    text = re.sub(r'```[a-z]*\s*', '', text).strip()
    m = re.search(r'\{[\s\S]+\}', text)
    if m:
        raw = m.group(0)
...
3. RECONSTRUCTION DES VARIABLES MATHÉMATIQUES PERDUES : les formules et variables mathématiques (comme α, β, u_n, u_(n+1), v_n, f(x), séquences de la forme "u_n = ...", équations, etc.) sont souvent perdues lors de l'OCR et apparaissent comme des espaces vides. Tu DOIS les reconstruire intelligemment d'après le contexte du thème et des questions. Par exemple : si le thème est "suite récurrente" et qu'il y a des espaces vides dans "u_0 =  , u_1 =  ", déduis les valeurs probables d'après le contexte ou laisse un placeholder clair comme "u_0 = [valeur manquante]". Si le thème est statistique et qu'on parle de "valeurs manquantes  et  dans une série", écris "valeurs manquantes a et b" ou les vraies lettres si déductibles.
4. NE résous AUCUNE question, NE modifie PAS les valeurs numériques déjà présentes
5. Si l'exercice est introuvable dans le texte brut, écris "" pour cleaned_text
...
[
  {{
    ...
    "year": "année",
    "topic": "thème",
    "cleaned_text": "Texte nettoyé complet : données + questions numérotées. Variables mathématiques manquantes reconstruites selon le contexte."
  }}
]
...
RAPPEL : JSON uniquement, pas de texte avant ou après."""
    ...
    raw_out = _call(prompt, max_tokens=3000).strip()
...
7. CRITIQUE : Le site BacIA possède un Cours Interactif complet pour TOUTES les matières listées ci-dessus. Tu dois TOUJOURS diriger {name} vers ces cours. Ne dis JAMAIS que les cours ne sont pas disponibles ou qu'ils arrivent bientôt — c'est faux. Si une matière apparaît dans les ressources ci-dessus avec "COURS INTERACTIF", c'est qu'il est pleinement accessible maintenant.
8. Les pourcentages de maîtrise proviennent des vraies performances de {name} sur le site. Présente-les avec confiance comme des données réelles (ex: "tu as 15% en maths" et non "tu aurais 15% environ")."""
    ...
    try:
        ...
        raw = (_resp.choices[0].message.content or '').strip() if _resp.choices else ''
        if not raw:
            raise ValueError("empty response")
    except Exception as _coach_err:
        import sys
        print(f"[smart_coach] json_object call failed: {_coach_err}", file=sys.stderr)
        raw = _call_json(prompt, max_tokens=3000).strip()
...
def course_chat(
    ...
    image_mime: str = None,
    subject_score: int | None = None,  # score DB 0-100 pour cette matière
) -> dict:
    """
...
╚══════════════════════════════════════════════════════════════╝"""
    ...
    # Course tutoring uses Pro model for maximum quality.
    ...
    # Bias depuis le score DB quand la conversation est trop courte
    if subject_score is not None:
        _student_msg_count = sum(
            1 for m in (messages or [])
            if m.get('role') not in ('ai', 'assistant', '__plan__', '__plan_intro__', '__chunk_state__')
        )
        if _student_msg_count < 3:
            # Pas assez de données conversation → utiliser le score DB
            if subject_score < 40:
                detected_level = 'faible'
            elif subject_score >= 70:
                detected_level = 'avancé'
            else:
                detected_level = 'moyen'
    ...
    if _student_intent == 'other':
        if teach_idx >= total:
            step_instructions = (
                "MISSION — Répondre à la question libre de l'élève\n"
                f"L'élève pose une question libre après la fin du cours : « {user_message} »\n"
                "RÈGLES DE SORTIE OBLIGATOIRES (visibles élève):\n"
                "- Réponds DIRECTEMENT, précisément et pédagogiquement à sa question.\n"
                "- Si la réponse est dans les notes fournies, utilise-les en priorité absolue.\n"
                f"- Si la réponse n'est pas dans les notes fournies mais concerne la matière {subject_label}, tu es EXCEPTIONNELLEMENT autorisé à utiliser tes connaissances du programme officiel du BAC Haïtien pour lui répondre précisément.\n"
                "- Sois clair, concis (max 150-250 mots).\n"
                "- Aucun titre, aucun label interne visible.\n"
            )
        else:
            step_instructions = (
                "MISSION — Répondre à la question libre de l'élève\n"
                f"L'élève pose une question ou fait une remarque libre : « {user_message} »\n"
                "RÈGLES DE SORTIE OBLIGATOIRES (visibles élève):\n"
                "- Réponds DIRECTEMENT, précisément et pédagogiquement à sa question.\n"
                "- Si la réponse est dans les notes fournies, utilise-les en priorité absolue.\n"
                f"- Si la réponse n'est pas dans les notes fournies mais concerne la matière {subject_label}, tu es EXCEPTIONNELLEMENT autorisé à utiliser tes connaissances du programme officiel du BAC Haïtien pour lui répondre précisément.\n"
                "- Sois clair, concis (max 150-250 mots).\n"
                f"- À la fin de ta réponse, fais une transition douce pour ramener l'élève vers le cours actuel (Concept : {concept_to_teach}) et invite-le à continuer.\n"
                "- Aucun titre, aucun label interne visible.\n"
            )
    elif teach_idx >= total:
        step_instructions = (
        ...
        _level_hint = (
            "niveau faible: vocabulaire très simple, phrases courtes, reprendre les bases du concept avant d'aller plus loin"
            if detected_level == 'faible'
            else ("niveau avancé: rigoureux, précis, pas de sur-explication" if detected_level == 'avancé'
        ...
        step_instructions = (
            ...
            f"{_style_override}"
            "Ne termine PAS par une question systématique. "
            "Laisse l'élève réagir naturellement avec les boutons ou en écrivant."
        )
...
"""
    ...
    if has_notes:
        if _student_intent == 'other':
            source_rules = f"""SOURCE :
- Utilise les notes fournies ci-dessous (note_*.json) si la réponse s'y trouve.
- Si la question de l'élève n'est pas couverte dans les notes fournies mais concerne la matière {subject_label}, utilise tes connaissances approfondies du programme officiel du BAC Haïtien pour lui répondre précisément.
- N'invente pas de fausses méthodes ou de faits historiques erronés."""
        else:
...
- N'invente PAS de fausses informations."""
    ...
    if _student_intent == 'other':
        evidence_instruction = (
            ...
            "---EVIDENCE---\n"
            "[2-3 citations courtes tirées EXACTEMENT des notes, ou laisse ce bloc vide si la réponse provient de tes connaissances générales]\n"
            "---END-EVIDENCE---"
        )
    else:
        evidence_instruction = (
...
def course_chunk_clarification(
    *,
    ...
    # Niveau selon score DB
    if subject_score is not None:
        if subject_score < 40:
            _clarif_level = "faible: utilise des mots très simples, reprendre les bases si nécessaire, phrases courtes"
        elif subject_score >= 70:
            _clarif_level = "avancé: rigoureux et précis, va droit au but"
        else:
            _clarif_level = "intermédiaire: clair et structuré"
    else:
        _clarif_level = "normal: clair et structuré"
    ...
    prompt = (
        ...
        "- Ne termine PAS par une question systématique. Laisse l'élève réagir naturellement.\n"
        f"{style_hint}"
...
# ═══════════════════════════════════════════════════════════════════════════════
# EXAM UNIQUENESS — Appel DeepSeek pour varier les questions ouvertes
# ═══════════════════════════════════════════════════════════════════════════════

def ai_enhance_exam(exam_data: dict, subject: str) -> dict:
    """
    Appelle DeepSeek pour remplacer les questions ouvertes d'un examen assemblé
    par des questions ORIGINALES et UNIQUES au niveau BAC Haïti.

    - Pour les matières déjà traitées par IA dans generate_exam_from_db
      (philosophie, francais) : passe directement, évite double appel.
    - Pour toutes les autres : génère 3-5 nouvelles questions ouvertes variées.
    - Si l'appel échoue, l'examen original est retourné intact (non bloquant).
    """
    import json as _json
    import re as _re

    # Matières déjà traitées par IA dans generate_exam_from_db
    _ALREADY_AI = {'philosophie', 'francais'}
    if subject in _ALREADY_AI:
        return exam_data

    _SUBJECT_LABELS = {
        'maths': 'Mathématiques', 'physique': 'Physique', 'chimie': 'Chimie',
        'svt': 'SVT (Sciences de la Vie et de la Terre)', 'histoire': 'Histoire & Géographie',
        'economie': 'Économie', 'anglais': 'Anglais', 'espagnol': 'Espagnol',
        'informatique': 'Informatique', 'art': 'Arts Plastiques',
    }
    subject_label = _SUBJECT_LABELS.get(subject, subject.capitalize())

    # Extraire un aperçu des questions existantes pour guider l'IA
    existing_questions: list[str] = []
    for part in (exam_data.get('parts') or []):
        for sec in (part.get('sections') or []):
            for item in (sec.get('items') or [])[:2]:
                txt = (item.get('text') or '')[:120]
                if txt:
                    existing_questions.append(txt)
        if len(existing_questions) >= 6:
            break

    existing_preview = '\n'.join(f'- {q}' for q in existing_questions[:6])

    prompt = (
        f"Tu es professeur de {subject_label} au Bac Haïti Terminale (niveau {subject_label}).\n"
        "Génère EXACTEMENT 4 nouvelles questions ouvertes ORIGINALES et UNIQUES pour un examen blanc BAC Haïti.\n\n"
        f"Contexte (questions déjà présentes dans l'examen — NE PAS répéter) :\n{existing_preview}\n\n"
        "RÈGLES :\n"
        "1. Questions de niveau Terminale BAC Haïti — pas trop faciles, pas trop complexes.\n"
        "2. Chaque question doit porter sur un THÈME DIFFÉRENT du programme officiel.\n"
        "3. Inclure le barème (pts) : 4-5 pts par question (total 16-20 pts).\n"
        "4. Formules mathématiques entre $...$.\n"
        "5. Inclure une réponse/corrigé modèle concis pour chaque question.\n\n"
        "Réponds UNIQUEMENT en JSON valide (sans markdown) avec cette structure exacte :\n"
        '{"questions": ['
        '{"text": "Énoncé complet...", "answer": "Corrigé modèle...", "pts": 5},'
        '{"text": "...", "answer": "...", "pts": 5},'
        '{"text": "...", "answer": "...", "pts": 5},'
        '{"text": "...", "answer": "...", "pts": 5}'
        ']}'
    ...
    system = (
        f"Tu es un professeur expert de {subject_label} niveau Terminale BAC Haïti. "
        "Réponds UNIQUEMENT avec du JSON valide, sans texte avant ou après."
    )

    try:
        raw = _call_json(prompt, system=system, max_tokens=1800)
        raw = _re.sub(r'```[a-z]*\s*', '', raw).strip().strip('`')
        m = _re.search(r'\{[\s\S]*\}', raw)
        if not m:
            return exam_data
        new_qs = _json.loads(m.group(0)).get('questions', [])
        if not new_qs or len(new_qs) < 2:
            return exam_data
    except Exception:
        return exam_data  # Non-bloquant : retourner l'examen original si l'IA échoue

    # Injecter les nouvelles questions dans la DERNIÈRE section de la DERNIÈRE partie
    try:
        parts = exam_data.get('parts') or []
        if not parts:
            return exam_data
        last_part = parts[-1]
        sections = last_part.get('sections') or []
        if not sections:
            # Créer une nouvelle section si nécessaire
            new_section = {
                'label': 'Questions supplémentaires',
                'type': 'open',
                'pts': sum(q.get('pts', 5) for q in new_qs),
                'items': [{'text': q['text'], 'answer': q.get('answer', ''), 'pts': q.get('pts', 5)} for q in new_qs],
            }
            last_part['sections'] = [new_section]
        else:
            last_sec = sections[-1]
            items = last_sec.get('items') or []
            # Remplacer les 4 derniers items (ou ajouter si moins de 4 items)
            replacement = [{'text': q['text'], 'answer': q.get('answer', ''), 'pts': q.get('pts', 5)} for q in new_qs]
            if len(items) >= 4:
                last_sec['items'] = items[:-4] + replacement
            else:
                last_sec['items'] = replacement
            # Recalculer les points de la section
            last_sec['pts'] = sum(it.get('pts', 0) for it in last_sec['items'])
    except Exception:
        pass  # Non-bloquant
