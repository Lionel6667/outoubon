...
# ─── Cache des examens générés (partage inter-utilisateurs) ─────────────────
class GeneratedExam(models.Model):
    """
    Stocke chaque examen généré en DB pour éviter les appels API répétitifs.
    Logique :
      1. Avant de générer : chercher un examen non encore vu par cet utilisateur.
      2. Si trouvé : servir directement, marquer comme vu.
      3. Si épuisé : appeler DeepSeek, stocker, marquer comme vu.
    """
    subject    = models.CharField(max_length=50, db_index=True)
    serie      = models.CharField(max_length=20, blank=True, default='', db_index=True)
    exam_data  = models.JSONField()
    created_at = models.DateTimeField(auto_now_add=True)
    seen_by    = models.ManyToManyField(
        User, blank=True, related_name='seen_generated_exams'
    )
