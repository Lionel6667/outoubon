...
def get_subject_chapters(subject: str) -> list[str]:
    ...
    try:
        ...
        if isinstance(data, dict) and 'raw_text' in data:
            ...
            # Format {"course_title": "...", "chapters": [{"chapter_title": "...", ...}]}
            chapters = []
            for item in data['chapters']:
                if isinstance(item, dict):
                    t = (item.get('chapter_title') or item.get('title')
                         or item.get('chapitre') or item.get('nom', ''))
                    if t:
...
def get_full_resource_catalog() -> str:
    ...
    for subject, label in SUBJECT_LABELS.items():
        ...
        has_note_file = bool(NOTE_FILES.get(subject) and (DB_DIR / NOTE_FILES[subject]).exists()
                             and (DB_DIR / NOTE_FILES[subject]).stat().st_size > 100)

        if not cats and not chapters and not has_note_file:
            continue
        ...
        if chapters:
            lines.append(f"  COURS INTERACTIF — chapitres disponibles : {', '.join(chapters[:10])}")
        elif has_note_file:
            lines.append(f"  COURS INTERACTIF : disponible (contenu complet chargé)")
        else:
            lines.append(f"  COURS INTERACTIF : non disponible pour cette matière")
